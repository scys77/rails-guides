...
Active Support 是 Ruby on Rails 的组件，用来提供 Ruby 扩展、实用程序以及其它一些横向相关的东西。

它提供了语言级别上的更丰富的基础功能，既适用于开发 Rails 应用程序，也适用于开发 Ruby on Rails 本身。

在这篇指南里你将学到的是 Active Support 对 Ruby 核心类和模块的扩展。
...
为了占用的空间尽可能地小，Active Support 默认不加载任何东西。它不好用的时候也是一小块一小块的，所以你想用什么就尽管加载进来，它提供了很方便的入口点一次加载所有相关扩展，甚至是全部。

因此，如果简单地写了这样一个 require ：
...
对象还不能拥有 +blank?+ 方法。我们来看看如何加载这个方法的定义：

h5. 选取一个方法定义

获得 +blank?+ 的最轻量级方法是选取定义它的文件。

本指南会对每一个提到的内核扩展方法都提示出它的定义位置。例如 +blank?+ 会有提示：
...
Active Support 已经被认真地修订为当选取一个文件时，只会加载确实存在相关依赖的东西。

h5. 加载一组核心扩展

下一个级别是加载所有对 +Object+ 的扩展。从经验来讲，如果想一次性使对 +SomeClass+ 的扩展都可用，只需加载 +active_support/core_ext/some_class+。

于是，加载所有对 +Object+ 的扩展（包括 +blank?+）的方法是：
...
你可能更想一次就把所有核心扩展都加载进来，确实有一个文件就是用来做这个的：

<ruby>
require 'active_support/core_ext'
</ruby>

h5. 加载全部 Active Support

最后，如果你想让全部 Active Support 都可用，则：

<ruby>
require 'active_support/all'
</ruby>

它并不会把整个 Active Support 放入内存，即使接下来确实会用到，因为很多地方都被配置为 +autoload+，所以只有用到的时候才会被加载。

h4. Ruby on Rails 应用程序里的 Active Support

一个 Ruby on Rails 应用程序会加载整个 Active Support，除非 +config.active_support.bare+ 为 true。如果设置为 true 了，应用程序只会加载框架自己按自身需要选取的东西，并且可以选取前面提到的各个粒度的。
